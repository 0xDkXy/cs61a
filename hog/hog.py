...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_val = avg(num, dice)
    id = 1
    num += 1
    while num <= 10:
        avg_val = avg(num, dice)
        if avg_val > max_val:
            max_val = avg_val
            id = num
        num += 1
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    return 0 if val >= threshold else num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # return 6  # Remove this line once implemented.
    hefty_hogs_val = hefty_hogs(score, opponent_score)
    val = hog_pile(score + hefty_hogs_val, opponent_score)
    return 0 if val > 0 or val + hefty_hogs_val >= threshold else num_rolls
    # END PROBLEM 11
